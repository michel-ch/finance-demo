...
window.FC.ForecastScreen = function ForecastScreen({ blurred, data, displayFont }) {
  ...
  const history = data.forecast.history.map(p => ({ ...p, v: p.v * accountFactor }));
  const projection = data.forecast.projection
    ...
    <div style={{
      ...
      {sim.enabled && (
        <>
          ...
          <SimField label="In">
            <input
              type="range" min={1} max={60} value={sim.day}
              onChange={e => setSim(s => ({ ...s, day: +e.target.value }))}
              style={{ width: 120, accentColor: 'var(--negative)' }}
            />
            <span className="tabular" style={{ fontSize: 13, fontWeight: 600, minWidth: 32 }}>{sim.day}d</span>
          </SimField>
