  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: compact ? 20 : 28 }}>
      ...
      >
        <div style={{
          ...
          {data.accounts.map(acc => (
            <AccountCard key={acc.id} account={acc} blurred={blurred} onNav={onNav} />
          ))}
        </div>
      </Section>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: 20 }}>
        <Section
          title="Goals"
          subtitle={`${data.goals.length} active${data.goals.filter(g => g.status === 'slipping' || g.status === 'off-track').length ? ' · ' + data.goals.filter(g => g.status === 'slipping' || g.status === 'off-track').length + ' needs attention' : ''}`}
  ...
  return (
    <div className="fc-card" style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <span style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>{data.accounts.length} accounts</span>
      </div>
...
function ForecastPeekCard({ data, blurred, onNav }) {
  const projection = (data.forecast && data.forecast.projection) || [];
  const events = (data.forecast && data.forecast.events) || [];
  if (!projection.length) {
    return (
      <button className="fc-card" onClick={() => onNav('forecast')} style={{
        padding: 24, textAlign: 'left', display: 'flex', flexDirection: 'column', gap: 8, cursor: 'pointer',
      }}>
        <div style={{ fontSize: 11, color: 'var(--text-tertiary)', fontWeight: 600, letterSpacing: '0.6px', textTransform: 'uppercase' }}>30-day forecast</div>
        <div style={{ fontSize: 18, fontWeight: 600, fontFamily: 'var(--font-display)' }}>Not enough data yet</div>
        <div style={{ fontSize: 13, color: 'var(--text-secondary)' }}>Add at least one recurring expense to unlock forecasting.</div>
        <div style={{ marginTop: 'auto', display: 'flex', alignItems: 'center', gap: 4, fontSize: 12, color: 'var(--accent)', fontWeight: 600, paddingTop: 8 }}>
  ...
  const low = projection.reduce((min, p) => p.v < min.v ? p : min, projection[0]);
    ...
    >
      ...
      <div>
        ...
        <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 4 }}>
          On <strong style={{ color: 'var(--text-primary)' }}>{lowDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</strong>
        </div>
      </div>
      ...
      <div style={{
        ...
        <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>{events.length} events queued</span>
...
function MiniForecast({ data, blurred, statusColor }) {
  const w = 360, h = 64;
  const hist = (data.forecast && data.forecast.history) || [];
  const proj = (data.forecast && data.forecast.projection) || [];
  if (!hist.length && !proj.length) return <div style={{ height: h }} />;
  const all = [...hist.slice(-20), ...proj.slice(0, 30)];
  const split = 20;
...
// ── Account card ────────────────────────────
function AccountCard({ account, blurred, onNav }) {
  ...
  return (
    <div className="fc-card" style={{
      ...
      onClick={() => { if (onNav) onNav('accounts'); else location.href = 'accounts.html'; }}
