...
window.FC.CardsScreen = function CardsScreen({ blurred, data }) {
  ...
  const totalLiab = credit.reduce((s, c) => s + (c.cycleSpend || 0), 0);
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: 24 }}>
          <div>
            ...
            <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 4 }}>0 cards</div>
          </div>
          <div style={{ flex: 1 }} />
          <button className="fc-btn fc-btn-primary" style={{ height: 34 }}
            onClick={() => window.dispatchEvent(new CustomEvent('fc:edit-card'))}>
            <Icon name="plus" size={14} /> Add card
          </button>
        </div>
        <div className="fc-card" style={{ padding: 48, textAlign: 'center' }}>
          <div style={{ fontSize: 32, marginBottom: 12 }}>💳</div>
          <div style={{ fontSize: 18, fontWeight: 600, marginBottom: 6, fontFamily: 'var(--font-display)' }}>Add a card</div>
          <div style={{ fontSize: 14, color: 'var(--text-secondary)' }}>
            {accounts.length ? 'Link a debit or credit card to one of your accounts.' : 'Add an account first, then attach cards to it.'}
          </div>
        </div>
      </div>
    );
  ...
  // Map account ids → names so settlement summary doesn't hardcode any.
  function settlementAccount() {
    if (!credit.length) return null;
    const ids = credit.map(c => c.accountId).filter(Boolean);
    const acc = accounts.find(a => ids.includes(a.id));
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <div style={{ display: 'flex', alignItems: 'flex-end', gap: 24 }}>
        <div>
          ...
          <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 4 }}>
            {cards.length} card{cards.length === 1 ? '' : 's'} · {credit.length} credit cycle{credit.length === 1 ? '' : 's'} pending settlement
          </div>
        </div>
        <div style={{ flex: 1 }} />
        <button className="fc-btn fc-btn-primary" style={{ height: 34 }}
      ...
      {credit.length > 0 && (
        <div className="fc-card" style={{ padding: 24, display: 'flex', gap: 32, alignItems: 'flex-start' }}>
          <div style={{ flex: 1 }}>
            ...
            <MoneyDisplay amount={-totalLiab} currency={baseCurrency} size="display" colorize blurred={blurred} />
            <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 6 }}>
              {settlementAccount()
                ? <>Settles against <b>{settlementAccount()}</b> on the dates below. The forecast already accounts for this.</>
                : 'The forecast already accounts for upcoming settlements.'}
            </div>
          </div>
          <div style={{ width: 360 }}>
            ...
            {credit.map(c => (
              <div key={c.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 0', borderTop: '1px solid var(--border)' }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 13, fontWeight: 500 }}>{c.name || 'Card'} {c.last4 ? '•• ' + c.last4 : ''}</div>
                  <div style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>
                    {c.dueDate ? new Date(c.dueDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : '—'}
                    {c.statementDate ? ' · cycle ends ' + new Date(c.statementDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : ''}
                  </div>
                </div>
                <MoneyDisplay amount={-(c.cycleSpend || 0)} currency={baseCurrency} size="body" colorize blurred={blurred} />
              </div>
            ))}
          </div>
        </div>
      ...
      {credit.length > 0 && <h2 style={{ margin: '4px 0 -8px', fontSize: 16, fontWeight: 600 }}>Credit cards</h2>}
      {credit.length > 0 && (
      ...
      {debit.length > 0 && <h2 style={{ margin: '4px 0 -8px', fontSize: 16, fontWeight: 600 }}>Debit cards</h2>}
      {debit.length > 0 && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 16 }}>
...
function CreditCardTile({ card, blurred }) {
  return (
    <div onClick={() => window.dispatchEvent(new CustomEvent('fc:edit-card', { detail: { id: card.id } }))}
      style={{
      borderRadius: 14,
      padding: 20,
      color: '#fff',
...
function DebitCardTile({ card, blurred }) {
  return (
    <div className="fc-card" style={{ padding: 18, display: 'flex', alignItems: 'center', gap: 16, cursor: 'pointer' }}
      onClick={() => window.dispatchEvent(new CustomEvent('fc:edit-card', { detail: { id: card.id } }))}>
      <CardChip kind={card.network} />
      <div style={{ flex: 1 }}>
...
window.FC.SimulatorScreen = function SimulatorScreen({ data, blurred, displayFont }) {
  ...
  const [from, setFrom] = React.useState(accounts[0] ? accounts[0].id : '');
  const [date, setDate] = React.useState(new Date(Date.now() + 14 * 86400000).toISOString().slice(0, 10));
  ...
  if (!accounts.length) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        <div className="fc-card" style={{ padding: 48, textAlign: 'center' }}>
          <div style={{ fontSize: 32, marginBottom: 12 }}>💸</div>
          <div style={{ fontSize: 18, fontWeight: 600, marginBottom: 6, fontFamily: 'var(--font-display)' }}>Add an account first</div>
          <div style={{ fontSize: 14, color: 'var(--text-secondary)', marginBottom: 18 }}>The simulator needs at least one account to run hypotheticals against.</div>
  ...
  const acc = accounts.find(a => a.id === from) || accounts[0];
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button
              ...
              onClick={() => {
                const name = prompt('Name this scenario:', `${formatMoney(amount, 'EUR')} on ${date}`);
                if (!name) return;
                const key = 'fc.simulator.scenarios';
                let saved = [];
                try { saved = JSON.parse(localStorage.getItem(key) || '[]'); } catch (e) { saved = []; }
                saved.push({ name, amount, from, date, recurring, savedAt: new Date().toISOString() });
                localStorage.setItem(key, JSON.stringify(saved));
                alert(`Saved "${name}". ${saved.length} scenario${saved.length === 1 ? '' : 's'} stored.`);
              }}
            ...
            <button
              ...
              onClick={() => {
                if (recurring) {
                  ...
                  window.dispatchEvent(new CustomEvent('fc:recurring-saved'));
                  alert('Recurring rule added. View it on the Recurring page.');
                } else {
                  if (!confirm(`Add a planned transaction of ${formatMoney(amount, 'EUR')} on ${date}?`)) return;
                  window.FCStore.create('transactions', {
                    accountId: from,
                    account: acc.name,
                    description: 'Simulator: planned purchase',
                    merchant: 'Simulator: planned purchase',
                    tags: [],
                    date,
                    amount: -Math.abs(amount),
                  ...
                  alert('Planned transaction added (marked Pending). View it on the Transactions page.');
                }
              }}
            >Add as planned tx</button>
          </div>
        </div>
      </div>
    </div>
