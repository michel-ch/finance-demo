...
FC.MoneyDisplay = function MoneyDisplay({
  ...
  if (originalAmount != null && originalCurrency && originalCurrency !== currency) {
    if (blurred) {
      const origStr = FC.formatMoney(originalAmount, originalCurrency, { signed });
      const dotCount = Math.max(4, Math.round(origStr.length * 0.7));
      return (
        <span style={{ display: 'inline-flex', flexDirection: 'column', alignItems: 'flex-end' }}>
          <span className="tabular fc-money-blurred" style={{ ...sty, color }}>{'•'.repeat(dotCount)}</span>
          <span className="tabular fc-money-blurred" style={{ fontSize: 11, lineHeight: '14px', color: 'var(--text-tertiary)', fontWeight: 500 }}>≈ ••••</span>
        </span>
      );
    }
    return (
